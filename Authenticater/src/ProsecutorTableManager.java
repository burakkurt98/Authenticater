...
public class ProsecutorTableManager  implements TableManager{
	...
	public String getValues(String userName) {
		...
		ResultSet res = null;
		try {
		String sqlData = "select * from savci where id='" + userName + "'";
		res = name(sqlData);
		while(res.next()) {
	...
	public void singUp(PersonalInformation personalInformation) {
		try {
			Statement myStat = (Statement) (Statement) ((Statement) DatabaseConnectionSingleton.getInstance().getConnection());
	...
	public String insertInTo(PersonalInformation personalInformation) {
		...
		return "INSERT INTO savci (name,surname,id,password) VALUES ('" + personalInformation.getName() + "','" + personalInformation.getSurname() + "','" + personalInformation.getUsername() 
